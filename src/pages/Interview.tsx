...
export default function Interview() {
  ...
  //////////////////////////////////////////////////////
  // 🚀 START (UPDATED: difficulty-based)
  //////////////////////////////////////////////////////
  ...
  let { data } = await supabase
    ...
    .select("*")
    .eq("role", selectedRole)
    .eq("topic", currentQ.topic)
    .eq("subtopic", currentQ.subtopic)
    .gt("difficulty", currentQ.difficulty || 1)
    .limit(3);

  // 🔥 fallback 1 → ignore subtopic
  ...
  if (!data || data.length === 0) {
    alert("No deeper questions found ⚠️");
    return;
  }

  const existingIds = new Set(questions.map((q) => q.id));
  const filtered = data.filter((q) => !existingIds.has(q.id));
...
Return ONLY in JSON format:
{
  ...
  "improvements": "short text"
    ...
    //////////////////////////////////////////////////////
    // 🧠 AUTO ADAPTIVE LOGIC (FIXED)
    //////////////////////////////////////////////////////
    try {
      const currentQ = questions[currentIndex];

      let targetDifficulty = currentQ.difficulty || 1;

      if (parsed.score >= 8) targetDifficulty += 1;
      else if (parsed.score <= 4)
        targetDifficulty = Math.max(1, targetDifficulty - 1);

      let { data: nextQs } = await supabase
        .from("interview_questions")
        .select("*")
        .eq("role", selectedRole)
        .eq("topic", currentQ.topic)
      ...
      if (nextQs && nextQs.length > 0) {
        const existingIds = new Set(questions.map((q) => q.id));
        const filtered = nextQs.filter((q) => !existingIds.has(q.id));
      ...
      console.log("Adaptive fetch failed", e);
    }
    ...
    <div className="min-h-screen text-white flex justify-center p-6">
      <div className="w-full max-w-3xl space-y-6">
        ...
        <div className="text-xs text-purple-400">
          {q.topic} • {q.subtopic} • Level {q.difficulty}
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            ...
            {!feedback ? (
              ...
              <>
                <button onClick={handleDeepDive} className="px-4 py-2 bg-purple-600 rounded">
                  🔍 Dive Deeper
                </button>
