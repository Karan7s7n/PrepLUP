...
export default function Result() {
  ...
  useEffect(() => {
    ...
    const fetchResult = async () => {
      try {
        ...
        const index = allUsers?.findIndex((u) => u.user_id === user.id) ?? -1;
        setRank(index !== -1 ? index + 1 : null);

        if (acc < 40) {
          setAiInsight("You need strong fundamentals. Focus on basics and accuracy.");
        } else if (acc < 70) {
          setAiInsight("You're improving. Focus on consistency and reducing mistakes.");
        } else if (acc < 90) {
          setAiInsight("Great performance! Try harder questions to level up.");
        } else {
          setAiInsight("Excellent performance 🚀 You're among top performers.");
        }
      } catch (err) {
        console.error(err);
      }
  ...
  const maxPossibleScore = result.total_questions * 20;
  const scorePercent = Math.round((result.score / maxPossibleScore) * 100);
    ...
    <div className="min-h-screen px-6 pt-28 pb-10">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <motion.div
          ...
          className="p-10 rounded-3xl bg-gradient-to-r from-purple-600 to-blue-500 text-white"
        >
          <h1 className="text-4xl font-bold mb-2">Your Results 🎯</h1>
          <p className="opacity-80">Here's your real performance breakdown</p>
        </motion.div>
        ...
        <div className="p-6 rounded-3xl bg-white/5 border border-white/10">
          ...
          <p className="text-sm opacity-80 leading-relaxed">{aiInsight}</p>
        </div>
